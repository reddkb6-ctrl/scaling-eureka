...
@bot.tree.command(name="card", description="Get a random trading card!")
@app_commands.allowed_installs(guilds=True, users=True)
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
async def card(interaction: discord.Interaction):
    card_index = random.randint(0, len(cards) - 1)
    add_card(interaction.user.id, card_index)
    c = cards[card_index]

    embed = discord.Embed(title=c["name"], color=discord.Color.gold())
    embed.set_image(url=c["url"])
    embed.set_footer(text=f"Added to {interaction.user.name}'s collection!")
...
async def collection(interaction: discord.Interaction):
    owned = user_cards.get(interaction.user.id, [])
    ...
    for card_index in owned[:10]:
        c = cards[card_index]
        embed.add_field(name=c["name"], value="Owned ✅", inline=False)

    await interaction.response.send_message(embed=embed)

@bot.tree.command(name="givecard", description="Give a card to someone")
@app_commands.allowed_installs(guilds=True, users=True)
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
async def givecard(interaction: discord.Interaction, user: discord.User):
    giver_cards = user_cards.get(interaction.user.id, [])

    if not giver_cards:
        return await interaction.response.send_message("You have no cards to give 😭")

    card_index = giver_cards.pop()
    user_cards.setdefault(user.id, []).append(card_index)

    await interaction.response.send_message(
        f"🎴 {interaction.user.mention} gave a card to {user.mention}!"
    )
